...
TreeNode* createNode(TreeNode* left, TreeNode* right, 
    ...
    // Копируем только size символов вручную, чтобы избежать лишних копирований
    for (uint16_t i = 0; i < size; ++i) {
        node->data[i] = str[i];
    }
    node->data[size] = '\0'; // Гарантируем нуль-терминатор
...
void saveToFile(const TreeNode* node, FILE* file) {
    if (!node) {
        ...
        std::fwrite(&null_marker, sizeof(uint8_t), 1, file);
        return;
    }

    uint8_t null_marker = 1;
    std::fwrite(&null_marker, sizeof(uint8_t), 1, file);
...
int main() {
    ...
    while (std::scanf("%s", input) == 1) {
        if (input[0] == '+') {
            ...
            std::printf("%s\n", dictionary.add(input, input_len, num) ? "OK" : "Exist");
        } 
        else if (input[0] == '-') {
            resetString(input);
            std::scanf("%s", input);
            ...
            std::printf("%s\n", dictionary.remove(input, input_len) ? "OK" : "NoSuchWord");
        } 
        else if (input[0] == '!') {
            std::scanf("%s", input);
            if (input[0] == 'S') {
                ...
                if (file) {
                    dictionary.exportData(file);
                    std::fclose(file);
                    std::printf("OK\n");
                } else {
                    std::printf("ERROR: Can't open file for writing\n");
                }
            } else {
                resetString(input);
                std::scanf("%s", input);
                ...
                if (file) {
                    dictionary.importData(file);
                    std::fclose(file);
                    std::printf("OK\n");
                } else {
                    std::printf("ERROR: Can't open file for reading\n");
                }
            }
        } 
        else {
            ...
            TreeNode* found = dictionary.search(input, input_len);
            if (found) {
                std::printf("OK: %lu\n", found->value);
            } else {
